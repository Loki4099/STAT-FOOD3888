...
varImpPlot(rf_cv_model)
```


```{r}
# Generate PDP for each class of Hypertension
pdp_class0 <- pdp::partial(rf_cv_model, pred.var = "AGEC", prob = TRUE, 
                           plot = FALSE, type = "classification", which.class = 1, rug = TRUE)

pdp_class1 <- pdp::partial(rf_cv_model, pred.var = "AGEC", prob = TRUE, 
                           plot = FALSE, type = "classification", which.class = 2, rug = TRUE)


# Set up color palette for the 2 classes
pal <- c("blue", "red")

# Plot using base R
plot(NA, xlim = range(normalized_data$AGEC), ylim = c(0, 1), 
     main = "PDP for AGEC Predicting Hypertension", ylab = "Partial Dependence", xlab = "AGEC")
