...
## 1.3 Logistic regression (without CV)
```{r}
...
## 1.4 Logistic regression (with CV)

```{r}
# Define the train control for 5-fold cross-validation
train_control <- trainControl(method = "cv", number = 5, savePredictions = "final")

# Fit the logistic regression model with 5-fold cross-validation
cv_model <- train(Hypertension ~ SOCIAL + SEX + AGEC + BMISC + SMKSTAT + ALCPER + FIBRPER + CHOPER + Identity + PS_Ratio, 
                  data = normalized_data, 
                  method = "glm", 
                  family = binomial(),
                  trControl = train_control)

# Retrieve the predictions from the cross-validation process
predictions <- cv_model$pred

# Create the confusion matrix using the predictions and actual values
conf_matrix <- confusionMatrix(predictions$pred, predictions$obs)


# To plot confusion matrix with ggplot2
...
```{r}
# Extract Accuracy
accuracy <- conf_matrix$overall['Accuracy'][[1]]
...
# Use gt to create a well-formatted table
metrics_df %>%
  gt() %>%
  tab_header(
...
## 2.2 Random Forest (with CV)

```{r}
train_control <- trainControl(method = "cv", number = 5, savePredictions = "final")  # 10-fold cross-validation

set.seed(123)
rf_cv_model <- train(Hypertension ~ SOCIAL + SEX + AGEC + BMISC + SMKSTAT + ALCPER + FIBRPER + CHOPER + Identity + PS_Ratio, 
                     data = normalized_data, 
                     method = "rf", 
                     trControl = train_control, 
                     tuneLength = 5,     # Number of different mtry values to try (for tuning mtry)
...
```{r}
# Extract the importance of variables from the final model
importance_values <- varImp(rf_cv_model)

# Plot the importance of variables
plot(importance_values, main = "Variable Importance - Random Forest")



# Extract the Random Forest model from the caret train object
final_rf_model <- rf_cv_model$finalModel

# Use varImpPlot on the extracted randomForest model
...
variables <- c("AGEC", "BMISC", "CHOPER", "PS_Ratio", "FIBRPER", "ALCPER")
...
for (var in variables) {
  ...
  pdp_class0 <- pdp::partial(rf_cv_model, pred.var = var, prob = TRUE, 
                             plot = FALSE, type = "classification", which.class = 1, rug = TRUE)
  
  # Generate PDP for class 1 (Hypertension = 1)
  pdp_class1 <- pdp::partial(rf_cv_model, pred.var = var, prob = TRUE, 
