...
preprocess_params <- preProcess(age2, method = c("center", "scale"))
...
preprocess_params <- preProcess(age3, method = c("center", "scale"))
...
set.seed(3888)
results_age1 <- dia_cv(normalized_age1)
results_age2 <- dia_cv(normalized_age2)
results_age3 <- dia_cv(normalized_age3)
...
```{r age model}
age_model_1 <- gam(Systolic ~ s(CarbohydrateEnergy) + s(FiberEnergy), data = normalized_age1)
age_model_2 <- gam(Systolic ~ s(CarbohydrateEnergy) + s(FiberEnergy), data = normalized_age2)
age_model_3 <- gam(Systolic ~ s(CarbohydrateEnergy) + s(FiberEnergy), data = normalized_age3)

age_model_4 <- gam(Diastolic ~ s(CarbohydrateEnergy) + s(FiberEnergy), data = normalized_age1)
age_model_5 <- gam(Diastolic ~ s(CarbohydrateEnergy) + s(FiberEnergy), data = normalized_age2)
age_model_6 <- gam(Diastolic ~ s(CarbohydrateEnergy) + s(FiberEnergy), data = normalized_age3)

```

#### FiberEnergy
```{r age fiberenergy(Figure10), fig.width= 12, fig.height=14}
par(mfrow = c(3, 2))
...
### Group
```{r bmi group}
normalized_data_light <- selected_data %>% filter(BMI < 24.9)

normalized_data_normal <- selected_data %>% filter(BMI >= 24.9 & BMI <= 29.9)

normalized_data_heavy <- selected_data %>% filter(BMI > 29.9)
...
```{r bmi result}
set.seed(3888)
results_light <- sys_cv(normalized_data_light)
results_normal <- sys_cv(normalized_data_normal)
results_heavy <- sys_cv(normalized_data_heavy)
...
set.seed(3888)
results_light_dias <- dia_cv(normalized_data_light)
results_normal_dias <- dia_cv(normalized_data_normal)
results_heavy_dias <- dia_cv(normalized_data_heavy)
...
```{r bmi model}
bmi_model_1 <- gam(Systolic ~ s(CarbohydrateEnergy) + s(FiberEnergy), data = normalized_data_light)
bmi_model_2 <- gam(Systolic ~ s(CarbohydrateEnergy) + s(FiberEnergy), data = normalized_data_normal)
bmi_model_3 <- gam(Systolic ~ s(CarbohydrateEnergy) + s(FiberEnergy), data = normalized_data_heavy)

bmi_model_4 <- gam(Diastolic ~ s(CarbohydrateEnergy) + s(FiberEnergy), data = normalized_data_light)
bmi_model_5 <- gam(Diastolic ~ s(CarbohydrateEnergy) + s(FiberEnergy), data = normalized_data_normal)
bmi_model_6 <- gam(Diastolic ~ s(CarbohydrateEnergy) + s(FiberEnergy), data = normalized_data_heavy)

```

#### FiberEnergy
```{r age fiberenergy(Figure14), fig.width= 12, fig.height=14}
par(mfrow = c(3, 2))
...
### Group
```{r alcohol group}
normalized_data_nonalcohol <- selected_data %>% filter(AlcoholPercentage == 0)

normalized_data_alcohol <- selected_data %>% filter(AlcoholPercentage > 0)
...
set.seed(3888)
results_nonalcohol <- sys_cv(normalized_data_nonalcohol)
results_alcohol <- sys_cv(normalized_data_alcohol)
...
colors_alcohol <- c('#1b9e77', '#d95f02')
...
set.seed(3888)
results_nonalcohol_dias <- dia_cv(normalized_data_nonalcohol)
results_alcohol_dias <- dia_cv(normalized_data_alcohol)
...
```{r alcohol model}
alc_model_1 <- gam(Systolic ~ s(CarbohydrateEnergy) + s(FiberEnergy), data = normalized_data_nonalcohol)
alc_model_2 <- gam(Systolic ~ s(CarbohydrateEnergy) + s(FiberEnergy), data = normalized_data_alcohol)
alc_model_3 <- gam(Diastolic ~ s(CarbohydrateEnergy) + s(FiberEnergy), data = normalized_data_nonalcohol)
alc_model_4 <- gam(Diastolic ~ s(CarbohydrateEnergy) + s(FiberEnergy), data = normalized_data_alcohol)
```

#### FiberEnergy
```{r alcohol fiberenergy(Figure18), fig.width= 12, fig.height=8}
par(mfrow = c(2, 2))
...
results_male <- dia_cv(normalized_male)
results_female <- dia_cv(normalized_female)
...
#### FiberEnergy
```{r gender fiberenergy(Figure22), fig.width= 12, fig.height=8}
par(mfrow = c(2, 2))
...
#### CarbohydrateEnergy
```{r gender carbonhydrateenergy(Figure23), fig.width= 12, fig.height=8}
par(mfrow = c(2, 2))
carb_color <- "red"
