...
# Create the individual plots with labels "A", "B", "C", "D"
...
# Add explanation text at the bottom left of the grid
...
# Arrange the plots with a main title and the explanation text
grid.arrange(
  arrangeGrob(
    Fibre_syst_plot, Fibre_dias_plot, carbo_syst_plot, carbo_dias_plot,
    nrow = 2,
    top = textGrob("Figure 5: Relationship Between Energy Intake from Fibre and Carbohydrate and Blood Pressure", 
                   gp = gpar(fontsize = 16))
  ),
  explanation,
...
SystolicFig <- subplot(Systolic_rmse, Systolic_mae, Systolic_r2, nrows = 1, shareY = FALSE) %>%
  layout(
    title = list(
      text = "Figure 6: Model Comparison (Systolic)",
    ...
    annotations = list(
      ...
      list(
        ...
        y = -0.45, # Slightly lower for spacing
        xref = "paper",
        yref = "paper",
        showarrow = FALSE,
        font = list(size = 12),
      ...
      list(
        ...
        x = 0, # Same x as the first caption
        y = -0.5, # Slightly lower for spacing
        xref = "paper",
        yref = "paper",
        showarrow = FALSE,
        font = list(size = 12),
...
DiastolicFig <- subplot(
  ...
  layout(
    title = list(
      text = "Figure 7: Model Comparison (Diastolic)",
      font = list(size = 16), # Slightly larger title font
      x = 0.5,
      y = 1.15  # Move the title further from the subplots
    ),
...
SystolicFig <- subplot(Systolic_rmse, Systolic_mae, Systolic_r2, nrows = 1, shareY = FALSE, titleX = TRUE) %>%
  layout(
    title = list(
      text = "Figure 8: Model Comparison (Systolic)",
      font = list(size = 16), # Slightly larger title font
      x = 0.5,
      y = 1.15  # Move the title further from the subplots
    ),
...
DiastolicFig <- subplot(Diastolic_rmse, Diastolic_mae, Diastolic_r2, nrows = 1, shareY = FALSE, titleX = TRUE) %>%
  layout(
    title = list(
      text = "Figure 9: Model Comparison (Diastolic)",
      font = list(size = 16), # Slightly larger title font
      x = 0.5,
      y = 1.15  # Move the title further from the subplots
    ),
...
# Add missing columns to parametric and smooth dataframes
...
# Combine parametric and smooth terms into one dataframe
model_results <- rbind(parametric_full, parametric_reduced, smooth_full, smooth_reduced)

# Create separate dataframes for full model and reduced model
...
# Create the summary table for the Full Model
full_model_results %>%
  gt() %>%
  tab_header(
...
# Create the summary table for the Reduced Model
...
AgeSystolicFig <- subplot(Systolic_rmse, Systolic_mae, Systolic_r2, nrows = 1, shareY = FALSE) %>%
  layout(
    title = list(
      text = "Figure 9: Model Comparison (Systolic)",
      font = list(size = 16), # Slightly larger title font
      x = 0.5,
      y = 1.15  # Move the title further from the subplots
    ),
