...
# PDP Plot
```{r}
# Define the variables for which you want to generate PDP
variables <- c( "CarbonhydrateEnergy", "FiberEnergy")

# Set up color palette for the two classes
pal <- c("blue", "red")

# Set up a 2x3 grid layout for plotting multiple PDPs
par(mfrow = c(2, 3))  # 2 rows, 3 columns

# Loop through each variable to plot the PDP for each one
for (var in variables) {
  
  # Generate PDP for class 0 (Hypertension = 0)
  pdp_class0 <- pdp::partial(rf_model, pred.var = var, prob = TRUE, 
                             plot = FALSE, type = "classification", which.class = 1, rug = TRUE)
  
  # Generate PDP for class 1 (Hypertension = 1)
  pdp_class1 <- pdp::partial(rf_model, pred.var = var, prob = TRUE, 
                             plot = FALSE, type = "classification", which.class = 2, rug = TRUE)
  
  # Plot each variable's PDP using base R
  plot(NA, xlim = range(scaled_data[[var]]), ylim = c(0, 1), 
       main = paste("PDP for", var, "Predicting Hypertension"), 
       ylab = "Partial Dependence", xlab = var)
  
  # Draw lines for both classes
  lines(pdp_class0, col = pal[1], lwd = 2)
  lines(pdp_class1, col = pal[2], lwd = 2)
  
  # Add legend to differentiate between the two classes
...
for (var in variables) {
  ...
  # Generate PDP for class 0 (Hypertension = 0)
  pdp_class0 <- pdp::partial(rf_young_model, pred.var = var, prob = TRUE, 
                             plot = FALSE, type = "classification", which.class = 1, rug = TRUE)
  
  # Generate PDP for class 1 (Hypertension = 1)
  pdp_class1 <- pdp::partial(rf_young_model, pred.var = var, prob = TRUE, 
                             plot = FALSE, type = "classification", which.class = 2, rug = TRUE)
  
  # Plot each variable's PDP using base R
  plot(NA, xlim = range(scaled_data_young[[var]]), ylim = c(0, 1), 
       main = paste("PDP for", var, "Predicting Hypertension"), 
       ylab = "Partial Dependence", xlab = var)
...
for (var in variables) {
  ...
  # Generate PDP for class 0 (Hypertension = 0)
  pdp_class0 <- pdp::partial(rf_old_model, pred.var = var, prob = TRUE, 
                             plot = FALSE, type = "classification", which.class = 1, rug = TRUE)
  
  # Generate PDP for class 1 (Hypertension = 1)
  pdp_class1 <- pdp::partial(rf_old_model, pred.var = var, prob = TRUE, 
                             plot = FALSE, type = "classification", which.class = 2, rug = TRUE)
  
  # Plot each variable's PDP using base R
  plot(NA, xlim = range(scaled_data_old[[var]]), ylim = c(0, 1), 
       main = paste("PDP for", var, "Predicting Hypertension"), 
       ylab = "Partial Dependence", xlab = var)
