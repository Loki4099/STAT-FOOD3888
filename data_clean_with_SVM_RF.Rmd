...
The summary statistic of missing values been shown below, we found there occurs some missingness in BMI and blood pressure measurements.
...
```
As observed from the heat map, we can tell that the most significant multicollinearity (disregarding blood pressure) is between energy percentage from carbohydrate and fibers, and between the sodium and potassium. So we decide to only choose one between each to do the regression imputation for missing values.
...
Next we checked the outliers in each numeric variables, we had observed that all the variables except the **AGEC** (which indicate the age of the person) contain outliers,
```{r,fig.height=10}
...
```{r}
one_hot_vars = dummyVars(~ . - Hypertension - ABSPID - ABSHID - BP_Category - SYSTOL - DIASTOL, data = normalized_data)

normalized_numeric = predict(one_hot_vars, newdata = normalized_data)

final_data = data.frame(normalized_numeric)
final_data$Hypertension = normalized_data$Hypertension  # Reattach the target variable
...
# Predict probabilities on the training data (or validation data if you have it)
# train_probabilities <- predict(svm_model, newdata = final_data, type = "prob")

# Compute the ROC curve
# roc_curve <- roc(final_data$Hypertension, train_probabilities[,"Yes"])

# Plot the ROC curve
# plot(roc_curve, main = "ROC Curve for SVM Model")
```

This Support Vector Machine model is not very effective. It uses a Radial Basis Function kernel, and determines the best parameters. With the selected "best" parameters, the models performance is indicated by a ROC of 60.57%, which shows the trade-off between the True Positive Rate (Sensitivity) and the False Positive Rate (1 - Specificity). A perfect classifier has a ROC of 100%. Thus, the model has low effectiveness at distinguishing positive and negative cases, as any model with less than 70% ROC is deemed inaccurate. It has very low specificity, meaning it struggles to identify true negatives. This model was validated using 10 fold cross validation. 
...
cleaned_data = normalized_data[, !(names(normalized_data) %in% c("ABSPID", "ABSHID", "SYSTOL", "DIASTOL", "BP_Category"))]


set.seed(123) 
rf_model = train(Hypertension ~ ., 
                     data = cleaned_data, 
                     method = "rf", 
                     trControl = ctrl, 
                     tuneLength = 5,
...
```
This model is above the before mentioned threshold of 70%, but also has extremely low specificity. 

These models will need to be improved. 
