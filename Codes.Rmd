...
rf2 = randomForest(Hypertension ~ SocialStatus + Gender + Age + BMI + SmokeStatus + AlcoholPercentage + FiberEnergy + CarbonhydrateEnergy + Identity + PotassiumSodiumRatio,data = normalized_data_young, ntree = 500, importance = TRUE, proximity = TRUE)
...
rf3 = randomForest(Hypertension ~ SocialStatus + Gender + Age + BMI + SmokeStatus + AlcoholPercentage + FiberEnergy + CarbonhydrateEnergy + Identity + PotassiumSodiumRatio, 
                     data = normalized_data_old, ntree = 500, importance = TRUE, proximity = TRUE)
```

```{r important variables, fig.height = 5, fig.width= 20}
importance_rf1 <- as.data.frame(importance(rf1))
importance_rf1$Variable <- rownames(importance_rf1)
importance_rf1 <- importance_rf1 %>% arrange(desc(MeanDecreaseGini))

p1 <- ggplot(importance_rf1, aes(x = reorder(Variable, MeanDecreaseGini), y = MeanDecreaseGini)) +
  geom_bar(stat = "identity", fill = "#f4978e") +
  coord_flip() +
  labs(title = "Variable Importance (Random Forest All)", x = "Variables", y = "Mean Decrease in Gini") +
  theme_minimal()

importance_rf2 <- as.data.frame(importance(rf2))
importance_rf2$Variable <- rownames(importance_rf2)
importance_rf2 <- importance_rf2 %>% arrange(desc(MeanDecreaseGini))

p2 <- ggplot(importance_rf2, aes(x = reorder(Variable, MeanDecreaseGini), y = MeanDecreaseGini)) +
  geom_bar(stat = "identity", fill = "#f4978e") +
  coord_flip() +
  labs(title = "Variable Importance (Random Forest Young)", x = "Variables", y = "Mean Decrease in Gini") +
  theme_minimal()

importance_rf3 <- as.data.frame(importance(rf3))
importance_rf3$Variable <- rownames(importance_rf3)
importance_rf3 <- importance_rf3 %>% arrange(desc(MeanDecreaseGini))

p3 <- ggplot(importance_rf3, aes(x = reorder(Variable, MeanDecreaseGini), y = MeanDecreaseGini)) +
  geom_bar(stat = "identity", fill = "#f4978e") +
  coord_flip() +
  labs(title = "Variable Importance (Random Forest Old)", x = "Variables", y = "Mean Decrease in Gini") +
  theme_minimal()

grid.arrange(
  p1, p2, p3, ncol = 3, 
  top = textGrob("Figure 6: Variable Importance for Three Random Forest Models", gp = gpar(fontsize = 16, fontface = "bold")))
```

```{r model for important variables}
...
rf5 = randomForest(Hypertension ~ BMI + Age + PotassiumSodiumRatio + FiberEnergy + CarbonhydrateEnergy,data = normalized_data_young, ntree = 500, importance = TRUE, proximity = TRUE)

rf6 = randomForest(Hypertension ~ BMI + Age + PotassiumSodiumRatio + FiberEnergy + CarbonhydrateEnergy,data = normalized_data_old, ntree = 500, importance = TRUE, proximity = TRUE)
```
...
pal <- c("#f4978e", "#3b9ad0")  # Adjust the colors as needed
...
for (variable in variables) {
  ...
  # Print the combined plot for the current variable
  print(combined_plot)
}
```